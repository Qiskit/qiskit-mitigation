...
class AddSpectatorPostCircuitBitFlipChecks(TransformationPass):
    ...
    def _add_standalone(
        ...
        for qubit, clbit in zip(spectator_qubits_ls, spec_creg, strict=True):
        ...
        for qubit, clbit in zip(spectator_qubits_ls, spec_ps_creg, strict=True):
            dag.apply_operation_back(Measure(), [qubit], [clbit])
    ...
    def _integrate_with_postsel(
        ...
        for node in all_topo_nodes:
            ...
            if node._node_id == barrier1_id:
                ...
                for q, clbit in zip(spec_q_new, spec_ps_creg, strict=True):
        ...
        for node in dag.topological_op_nodes():
            ...
            if ("xslow" in node.op.name) or ("rx" in node.op.name) or (node.op.name == "reset"):
                ...
                for block in node.op.blocks:
                    ...
                    qubit_map = {
                        block_qubit: qubit
                        for block_qubit, qubit in zip(block_dag.qubits, node.qargs, strict=True)
                    }
                ...
                if node.op.name == "measure" and len(node.cargs) == 1:
                    ...
                    for block in node.op.blocks:
                        ...
                        inner_map = {
                            ...
                            for bq, pq in zip(block_dag.qubits, node.qargs, strict=True)
                        }

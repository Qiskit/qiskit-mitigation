...
"""Extrapolation functions used for zero noise extrapolation (ZNE) methods."""
...
_NON_POLYNOMIAL_MODELS = frozenset({"fallback", "exponential", "double_exponential", "custom"})
...
def _process_extrapolated_expectation_values(
    noise_scaled_exp_vals: np.typing.ArrayLike,
    noise_scaled_standard_errors: np.typing.ArrayLike,
    observable_term: str,
    zne_noise_factors: Sequence[float],
    extrapolators: str | Sequence[str],
    extrapolated_noise_factors: float | int | np.typing.ArrayLike = 0,
    custom_fit: tuple[
        Callable[..., np.ndarray], list[float], tuple[float, ...] | tuple[list[float], ...]
    ]
    | None = None,
) -> tuple[float, float, str, np.typing.NDArray[np.float64], np.typing.NDArray[np.float64]]:
    ...
    extrapolated_noise_factors = np.array(extrapolated_noise_factors, dtype=np.float64)
    extrapolated_noise_factors = np.insert(extrapolated_noise_factors, 0, 0.0)
    noise_scaled_exp_vals = np.array(noise_scaled_exp_vals, dtype=np.float64)
    noise_scaled_standard_errors = np.array(noise_scaled_standard_errors, dtype=np.float64)
    ...
    extrapolated_values, extrapolated_stderr = _fit_extrapolation_models(
    ...
    zero_noise_exp_val, zero_noise_std, selected_extrap = _select_zne_extrapolated_result(
...
def _fit_extrapolation_models(
    ...
    standard_error: np.typing.ArrayLike,
    zne_noise_factors: Sequence[float],
    models: Sequence[str],
    extrapolated_noise_factor: float | np.typing.ArrayLike = 0,
    custom_fit: tuple[
        Callable[..., np.ndarray], list[float], tuple[float, ...] | tuple[list[float], ...]
    ]
    | None = None,
    ...
    for i, name in enumerate(names):
        fit_values[i], fit_stderrs[i] = _extrapolate(
            name, x_data, y_data, y_std, fit_stds, x_eval, fallback_idx, custom_fit
        )
...
def _select_zne_extrapolated_result(
    zne_values: np.typing.NDArray[np.float64],
    zne_std_errors: np.typing.NDArray[np.float64],
    observable_term: str,
    ...
    accepted_idx = int(accepted[0] if accepted.size else fallback_indices)
...
def _extrapolate(
    ...
    if name == "custom" and custom_fit is None:
        raise ValueError("``custom_fit`` cannot be None is chosen model is ``custom``")
    try:
        ...
        # get the covariances.
        if name == "custom" and custom_fit is not None:
            func, p0, bounds = custom_fit
        else:
...
def _poly(x: np.typing.ArrayLike, *coeffs: float) -> np.ndarray:
    """Polynomial model, coeffs should be ordered lowest-order-first."""
    return np.array(polyval(np.asarray(x, dtype=float), coeffs))


def _multi_exp(x: np.typing.ArrayLike, *params: float) -> np.ndarray:
    ...
    out = np.zeros_like(x)
    for amp, rate in zip(params[::2], params[1::2], strict=True):
...
def _as_noise_factors(nf: float | np.typing.ArrayLike | None) -> np.ndarray:
...
def _validate_noise_factors(
    ...
    for extrap in extrapolators:
        ...
        if len(noise_factors) < required:
            raise ValueError(f"{extrap} requires at least {required} noise_factors")

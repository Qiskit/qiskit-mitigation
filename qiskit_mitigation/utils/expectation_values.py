...
def executor_expectation_values(
    ...
    *,
    ...
    rescale_factors: dict[str, float] | None = None,
):
    ...
    Args:
        ...
        rescale_factors: Scale factor for each unique Pauli term found in the calculated observables.
            Typically used for readout mitigation ("TREX") correction factors. If ``None``, scaling factor will not be applied.
    ...
    Raises:
        ValueError: ``avg_axis`` contains negative values, repeated axes, or one of the last two (shots, bits) axes.
        ValueError: ``avg_axis`` contains ``meas_basis_axis``.
        ValueError: ``meas_basis_axis`` is ``None`` but ``len(basis_mapping) != 1``.
        ValueError: ``meas_basis_axis`` is negative or indexes one of the last two (shots, bits) axes of ``bool_array``.
    ...
    elif meas_basis_axis > bool_array.ndim - 3:
        raise ValueError(
            f"`meas_basis_axis` ({meas_basis_axis}) must index one of the leading axes of `bool_array`: the "
            f"last two axes are shots and bits, so it can be at most {bool_array.ndim - 3}."
        )
    if meas_basis_axis in avg_axis:
        raise ValueError(
            f"`avg_axis` cannot include `meas_basis_axis` ({meas_basis_axis}): each measurement basis is "
            "evaluated separately, so the axis indexing the measurement bases cannot be averaged over."
        )
    ...
    for meas_basis_idx, (basis, observables) in enumerate(basis_dict.items()):
        ...
        # Rescale factors are keyed by the caller's observable labels, so drop the virtual
        # bits appended internally for postselection/PEC (indices >= the basis width):
                ...
                [
                    rescale_factors[
                        _full_pauli_label(
                            "".join(
                                char
                                for char, q in zip(term_chars, indices, strict=True)
                                if q < basis.num_qubits
                            ),
                            [q for q in indices if q < basis.num_qubits],
                            basis.num_qubits,
                        )
                    ]
                    for term_chars, indices, _ in observable.to_sparse_list()
                ]
        ...
        if isinstance(gamma_factor, float):
            basis_gamma: float | np.ndarray = gamma_factor
        else:
            # The empirical gamma has one entry per remaining (non-averaged) axis, with the
            # measurement bases along `meas_basis_axis`. Select this basis' slice and append
            # an axis for the observables being evaluated (to match the axis in `means`):
            basis_gamma = np.take(gamma_factor, meas_basis_idx, axis=meas_basis_axis)[
                ..., np.newaxis
            ]
        means = basis_gamma * means
        # Propagate uncertainties:
        variances = (basis_gamma * standard_errs) ** 2
    ...
    mean_and_var_each_observable = list(
        zip(mean_each_observable.tolist(), var_each_observable.tolist(), strict=True)
    )
...
def _apply_postselect_mask(
    ...
    Args:
        ...
        basis_dict: This dict encodes how the data in `bool_array` should be used to estimate the desired list of Pauli observables.
            Similar to `basis_mapping` arg of `executor_expectation_values()`, but here Pauli components and coefficients must be represented as
...
def _validate_avg_axis(avg_axis: int | tuple[int, ...] | None, num_dims: int) -> tuple[int, ...]:
    ...
    if len(set(avg_axis)) != len(avg_axis):
        raise ValueError(f"`avg_axis` contains repeated axes: {avg_axis}.")
...
def _apply_pec_signs(
    ...
    Args:
        bool_array: Boolean array, presumably representing data from measured qubits.
            The last two axes are the number of shots and number of classical bits, respectively.
        basis_dict: This dict encodes how the data in `bool_array` should be used to estimate the desired list of Pauli observables.
            Similar to `basis_mapping` arg of `executor_expectation_values()`, but here `None` may not be used as a placeholder for the zero operator.
    ...
    for observable_index, observable in enumerate(observables):
        ...
        for basis in measure_paulis:
            ...
            for element_index, (observable_element, observable_coeff) in enumerate(
                zip(observable.paulis, observable.coeffs, strict=True)
            ):
...
def _bitarray_expectation_value(
    ...
    if rescale_each_observable is not None:
        # Flatten rescale factors of all the terms of all observables into a 1D array. The
        # per-observable lists may have different lengths, so this cannot be an ndarray reshape.
        rescale_each_term = np.array(
            [factor for obs_factors in rescale_each_observable for factor in obs_factors]
        )
...
def _full_pauli_label(term_chars: str, indices: Sequence[int], num_qubits: int) -> str:
    """Reconstruct the full label of a sparse observable term, with qubit 0 rightmost.

    Args:
        term_chars: The non-identity characters of the term, e.g. from ``to_sparse_list()``.
        indices: The qubit index each character acts on.
        num_qubits: The total number of qubits in the observable.

    Returns:
        The full label string, e.g. ``"IZX"``.
    """
    label = ["I"] * num_qubits
    for char, index in zip(term_chars, indices, strict=True):
        label[num_qubits - 1 - index] = char
    return "".join(label)


def _project_to_z(term: str) -> np.ndarray:
...
def _compute_single_term_exp_val_with_twirl_variance(
    ...
    """
    z_term = _project_to_z(observable_term)
    ...
    evals = np.prod(1 - 2 * datum[..., is_Z], axis=-1) if any_Zs else np.ones(datum.shape[:-1])

...
def test_get_measure_maps_raises_on_unknown_clbit():
    """``_get_measure_maps`` raises ``RuntimeError`` when a measurement writes to a clbit
    that does not belong to any of the primary, post-check, or pre-check registers."""
    qreg = QuantumRegister(2, "q")
    creg_primary = ClassicalRegister(1, "alpha")
    creg_unknown = ClassicalRegister(1, "unknown")  # not in any check group

    circuit = QuantumCircuit(qreg, creg_primary, creg_unknown)
    circuit.measure(qreg[0], creg_primary[0])
    circuit.measure(qreg[1], creg_unknown[0])  # writes to a register outside all maps

    dag = circuit_to_dag(circuit)
    primary_cregs = {"alpha": circuit.cregs[0]}

    with pytest.raises(RuntimeError, match="Classical bit not part of neither"):
        _get_measure_maps(dag, primary_cregs, {}, {})

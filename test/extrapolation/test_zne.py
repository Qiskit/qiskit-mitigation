...
class TestZNEInit(unittest.TestCase):
    """Tests for :meth:`ZNE.__init__`."""

    def test_zne_attributes_are_none_after_init(self):
        """``noise_factors`` and ``extrapolator`` must be None immediately after construction."""
        zne = ZNE()
        self.assertIsNone(zne.noise_factors)
        self.assertIsNone(zne.extrapolator)
    ...
    def test_inherits_mitigation_task_attributes(self):
        ...
        for attr in (
            ...
            self.assertIsNone(getattr(zne, attr), msg=f"{attr} should be None after __init__")
...
class TestZNEBoxCircuit(unittest.TestCase):
    """Tests for the ZNE override of :meth:`_box_circuit`."""
    ...
    def test_returns_quantum_circuit(self):
        ...
        boxed = self.zne._box_circuit(self.circuit, {})
    ...
    def test_raises_when_enable_measures_is_false(self):
        ...
        with self.assertRaises(ValueError):
            self.zne._box_circuit(self.circuit, {"enable_measures": False})

    def test_none_options_uses_zne_defaults(self):
        """``None`` options must fall back to the ZNE default of ``enable_gates=False``."""
        boxed = self.zne._box_circuit(self.circuit, None)
        self.assertIsInstance(boxed, QuantumCircuit)
        self.assertIn("_meas", [reg.name for reg in boxed.cregs])
...
class TestZNEPrepare(unittest.TestCase):
    """Tests for :meth:`ZNE.prepare`."""
    ...
    def test_noise_factors_saved_as_float_array(self):
        ...
        self.zne.prepare(self.circuit, self.observables, parameters=None, noise_factors=(1, 3))
        self.assertIsInstance(self.zne.noise_factors, np.ndarray)
        self.assertEqual(self.zne.noise_factors.dtype, np.dtype(float))
        np.testing.assert_allclose(self.zne.noise_factors, [1.0, 3.0])

    def test_extrapolator_not_saved_when_none(self):
        """``extrapolator`` must stay ``None`` when not requested at preparation."""
        self.zne.prepare(self.circuit, self.observables, parameters=None, extrapolator=None)
        self.assertIsNone(self.zne.extrapolator)
    ...
    def test_extrapolator_saved_when_valid(self):
        ...
        self.assertEqual(self.zne.extrapolator, ["linear"])
    ...
    def test_program_item_index_points_at_first_item(self):
        ...
        self.assertEqual(self.zne._program_item_index, 0)
        second = ZNE()
        second.prepare(
            self.circuit,
            self.observables,
    ...
    def test_parametric_circuit_broadcast_mode(self):
        ...
        self.assertEqual(len(program.items), 2)
        self.assertTrue(zne.broadcast_obs_and_params)
        self.assertIsNotNone(zne.param_basis_pairs)
    ...
    def test_raises_when_trex_and_projection_observables(self):
        ...
        trex = TREX()
        with self.assertRaises(ValueError):
            zne.prepare(qc, project_obs, parameters=None, trex=trex)
    ...
    def test_prepare_with_trex_and_normal_observables_edits_boxing_options(self):
        ...
        trex = TREX()
        program = zne.prepare(qc, obs, parameters=None, trex=trex)
        self.assertIsInstance(program, QuantumProgram)
        self.assertGreater(len(program.items), 0)


# ---------------------------------------------------------------------------
# ZNE._calculate_extrapolated_expectation_values  (static method)
# ---------------------------------------------------------------------------


class TestCalculateExtrapolatedExpectationValues(unittest.TestCase):
    """Tests for :meth:`ZNE._calculate_extrapolated_expectation_values`."""
    ...
    def _calculate(
        ...
        return ZNE._calculate_extrapolated_expectation_values(
    ...
    def test_custom_fit_success(self):
        ...
        zero_exp_vals = ZNE._calculate_extrapolated_expectation_values(
...
# ---------------------------------------------------------------------------
# ZNE.compute_expectation_value_zne  (static method)
# ---------------------------------------------------------------------------


class TestComputeExpectationValueZne(unittest.TestCase):
    """Tests for the static :meth:`ZNE.compute_expectation_value_zne`."""
    ...
    def _compute(self, item_results=None, **kwargs):
        ...
        return ZNE.compute_expectation_value_zne(
    ...
    def test_non_broadcast_raises_on_wrong_ndim_in_loop(self):
        ...
        with self.assertRaises(ValueError):
            ZNE.compute_expectation_value_zne(
                [good, bad, good],
                observables=_obs("Z"),
                param_shape=(),
                param_basis_pairs=[(((),), "Z")],
    ...
    def test_custom_fit_success(self):
        """``compute_expectation_value_zne`` works with custom_fit."""
...
class TestZNEPostprocess(unittest.TestCase):
    """Tests for :meth:`ZNE.postprocess`."""

    def _make_minimal_zne(self):
        """Return a ZNE instance with the minimum attributes set for postprocess."""

        zne = ZNE()
        zne.noise_factors = np.array(NOISE_FACTORS)
        zne.extrapolator = ["linear"]
        zne.observables = _obs("Z")
        zne.parameters = None
        zne.param_basis_pairs = [((0,), "Z")]
        zne.meas_bases = None
        zne.broadcast_obs_and_params = True
        zne.extrapolated_noise_factors = None
        return zne
    ...
    def test_raises_when_extrapolator_unset_and_not_given(self):
        ...
        with self.assertRaises(ValueError):
            ZNE().postprocess([], noise_factors=NOISE_FACTORS)
    ...
    def test_uses_saved_noise_factors_when_not_passed(self):
        ...
        zne = self._make_minimal_zne()
        with patch.object(ZNE, "compute_expectation_value_zne", return_value=None) as mock_fn:
            zne.postprocess(MagicMock())
            _, call_kwargs = mock_fn.call_args
            np.testing.assert_array_equal(call_kwargs["noise_factors"], list(zne.noise_factors))
    ...
    def test_uses_saved_extrapolator_when_not_passed(self):
        ...
        with patch.object(ZNE, "compute_expectation_value_zne", return_value=None) as mock_fn:
            zne.postprocess(MagicMock())
            _, call_kwargs = mock_fn.call_args
            self.assertEqual(call_kwargs["extrapolator"], list(zne.extrapolator))
    ...
    def test_override_noise_factors_via_postprocess_kwarg(self):
        ...
        with patch.object(ZNE, "compute_expectation_value_zne", return_value=None) as mock_fn:
            zne.postprocess(MagicMock(), noise_factors=override)
            _, call_kwargs = mock_fn.call_args
    ...
    def test_override_extrapolator_via_postprocess_kwarg(self):
        ...
        override = ["fallback"]
        with patch.object(ZNE, "compute_expectation_value_zne", return_value=None) as mock_fn:
            zne.postprocess(MagicMock(), extrapolator=override)
            _, call_kwargs = mock_fn.call_args
    ...
    def test_extrapolated_noise_factors_passthrough(self):
        ...
        override = [0.0, 1.0]
        with patch.object(ZNE, "compute_expectation_value_zne", return_value=None) as mock_fn:
            zne.postprocess(MagicMock(), extrapolated_noise_factors=override)
            _, call_kwargs = mock_fn.call_args
...
class TestZNEExtractItemsListData(unittest.TestCase):
    """Tests for the TREX warning branches in :meth:`ZNE._extract_items_list_data`."""
    ...
    def test_warns_when_trex_set_but_no_measure_noise_and_item_result_list(self):
        ...
        with self.assertWarns(UserWarning):
            results, noise_data = zne._extract_items_list_data(item_results, None)
    ...
    def test_calls_compute_noise_model_when_trex_has_calibration(self):
        ...
        with patch.object(TREX, "compute_noise_model", return_value=sentinel_noise_data):
            _, noise_data = zne._extract_items_list_data(result, None)
    ...
    def test_no_warning_when_trex_none_and_full_result(self):
        ...
        with warnings.catch_warnings():
            warnings.simplefilter("error")
            _item_results, noise_data = zne._extract_items_list_data(result, None)

        self.assertIsNone(noise_data)


# ---------------------------------------------------------------------------
# ZNE.create_instance_from_passthrough_data
# ---------------------------------------------------------------------------


class TestZNECreateInstanceFromPassthroughData(unittest.TestCase):
    """Tests for :meth:`ZNE.create_instance_from_passthrough_data`."""
    ...
    def test_happy_path_returns_zne_instance(self):
        """A fully-populated passthrough dict must produce a ZNE instance."""

        zne = ZNE.create_instance_from_passthrough_data(self._minimal_passthrough())
        self.assertIsInstance(zne, ZNE)
    ...
    def test_attributes_set_from_passthrough(self):
        ...
        zne = ZNE.create_instance_from_passthrough_data(passthrough)
        self.assertEqual(zne._program_item_index, 5)
        self.assertTrue(zne.broadcast_obs_and_params)
        np.testing.assert_array_equal(zne.noise_factors, [1.0, 3.0, 5.0])
        self.assertEqual(zne.extrapolator, ["linear"])
        self.assertIsNone(zne.extrapolated_noise_factors)
    ...
    def test_raises_when_observables_missing(self):
        ...
        with self.assertRaises(ValueError):
            ZNE.create_instance_from_passthrough_data(passthrough)
    ...
    def test_raises_when_trex_calibration_true_but_no_trex(self):
        ...
        with self.assertRaises(ValueError):
            ZNE.create_instance_from_passthrough_data(passthrough, trex=None)

...
def test_edge_mask_skips_edges_with_unmeasured_qubits_post():
    """Edge-based post mask skips an edge where one endpoint is absent from ``summary.measure_map``."""
    # qubit 2: in measure_map_ps only (no primary measurement)
    # edge (1, 2): qubit 1 in measure_map, qubit 2 only in measure_map_ps
    ...
    outer_shape = (4,)
    data = np.zeros((*outer_shape, 2), dtype=bool)
    data_ps = np.ones((*outer_shape, 3), dtype=bool)  # flipped for all 3 _ps bits
    result = {"alpha": data, "alpha_ps": data_ps}

    # All shots must pass: edge (0,1) passes (primary != _ps); edge (1,2) is skipped
    # because qubit 2 is absent from measure_map.
    mask = selector.compute_mask(result, strategy="edge", mode="post")
    assert mask.shape == outer_shape
    assert np.all(mask)


def test_edge_mask_skips_edges_with_unmeasured_qubits_pre():
    """Edge-based pre mask skips edges where one endpoint appears only in the post-check
    map but not in ``summary.measure_map_pre``.
    """
    # Build the summary directly: measure_map covers all 3 qubits; measure_map_pre
    # covers only 0 and 1.  Edge (1, 2) is in edges but qubit 2 is absent from
    # measure_map_pre — the pre-edge loop must ``continue`` past it.
    summary = PostSelectionSummary(
        primary_cregs={"alpha"},
    ...
    outer_shape = (4,)
    data = np.zeros((*outer_shape, 3), dtype=bool)
    data_pre = np.zeros((*outer_shape, 2), dtype=bool)  # all zeros → good initialization
    result = {"alpha": data, "alpha_pre": data_pre}

    # All shots must pass: edge (0,1) passes the pre-check; edge (1,2) is skipped.
    mask = selector.compute_mask(result, strategy="edge", mode="pre")
    assert mask.shape == outer_shape
    assert np.all(mask)

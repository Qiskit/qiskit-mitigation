...
from qiskit_mitigation.utils.measurement_bases import _convert_basis_to_uint_representation
from qiskit_mitigation.utils.observable_mappings import map_observable_virtual_to_canonical
...
class TestExecutorExpectationValuesInputValidation(unittest.TestCase):
    ...
    def test_avg_over_meas_basis_axis(self):
        """Test that including meas_basis_axis in avg_axis raises a clear ValueError.

        Previously this surfaced as a bare ``IndexError`` from numpy inside the per-basis loop.
        """
        bool_array, basis_dict = self._create_minimal_valid_inputs(num_bases=2)

        for avg_axis in (0, (0,)):
            with self.subTest(avg_axis=avg_axis):
                with self.assertRaises(ValueError) as context:
                    executor_expectation_values(
                        bool_array,
                        basis_dict,
                ...
                self.assertIn("cannot include `meas_basis_axis`", str(context.exception))

    def test_meas_basis_axis_out_of_range(self):
        """Test that meas_basis_axis on the shots/bits axes, or beyond, raises a clear ValueError.

        Previously these surfaced as a numpy ``IndexError`` or a misleading basis-count mismatch.
        """
        bool_array, basis_dict = self._create_minimal_valid_inputs(num_bases=2)
        ndim = bool_array.ndim  # (num_bases, shots, bits): only axis 0 is valid

        for meas_basis_axis in (ndim - 2, ndim - 1, ndim):
            with self.subTest(meas_basis_axis=meas_basis_axis):
                with self.assertRaises(ValueError) as context:
                    executor_expectation_values(
                        bool_array,
                        basis_dict,
                ...
                self.assertIn("must index one of the leading axes", str(context.exception))

    def test_repeated_avg_axis(self):
        """Test that repeated entries in avg_axis raise a clear ValueError.

        Previously this surfaced as numpy's "repeated axis in `source` argument".
        """
        bool_array, basis_dict = self._create_minimal_valid_inputs(num_bases=2)
        bool_array = bool_array[np.newaxis]  # (1, num_bases, shots, bits): axis 0 is averageable
        ...
        self.assertIn("repeated axes", str(context.exception))
    ...
    def test_negative_meas_basis_axis_raises(self):
        """A negative ``meas_basis_axis`` (but not None) must raise ValueError."""
        bool_array = np.zeros((2, 4, 2), dtype=bool)
        basis_mapping = {Pauli("ZZ"): [SparsePauliOp("ZZ")]}
        with self.assertRaisesRegex(ValueError, "meas_basis_axis must be nonnegative"):
            executor_expectation_values(bool_array, basis_mapping, meas_basis_axis=-1)
...
class TestExecutorExpectationValuesSimple(unittest.TestCase):
    ...
    def _synthesize_simple_basis_dict(self, bool_array, meas_axis=None, seed=None):
        ...
        basis_dict = dict(zip(meas_bases, obs_terms_each_basis, strict=True))
        return basis_dict
    ...
    def test_empirical_gamma_without_explicit_gamma_factor(self):
        """Providing ``pauli_signs`` without ``gamma_factor`` triggers the empirical gamma computation path."""
        num_shots = 100
        num_qubits = 2
        bool_array = np.random.randint(0, 2, size=(num_shots, num_qubits), dtype=bool)
        basis_dict = {Pauli("ZZ"): [SparsePauliOp("ZZ", coeffs=[1.0])]}
        ...
        result = executor_expectation_values(
            ...
            pauli_signs=pauli_signs,
            postselect_mask=postselect_mask,
            # No gamma_factor supplied → empirical path
        )
    ...
    def test_sparse_observable_in_basis_mapping_is_used_directly(self):
        """A ``SparseObservable`` in the basis_mapping values should be passed through
        directly without conversion."""
        bool_array = np.zeros((1, 4, 2), dtype=bool)
        # Build a SparseObservable directly and place it in the basis mapping
        sparse_obs = SparseObservable.from_sparse_pauli_op(SparsePauliOp("ZZ"))
        basis_mapping = {Pauli("ZZ"): [sparse_obs]}
        result = executor_expectation_values(
            bool_array,
            basis_mapping,
            meas_basis_axis=0,
        ...
        self.assertEqual(len(result), 1)
        # All-zeros data, ZZ operator → expectation value = +1
        np.testing.assert_allclose(result[0][0], 1.0, atol=1e-10)

    def test_projector_observable_all_shots_projected(self):
        """Observables with projector terms (0 or 1 terms) are computed correctly with
        all shots projected."""
        bool_array = np.zeros((1, 4, 1), dtype=bool)

        # Build a SparseObservable with a "0" projector (|0><0|) via coerce
        # We use the helper "0" label via from_list
        proj_obs = SparseObservable.from_list([("0", 1.0)])
        basis_mapping = {Pauli("Z"): [proj_obs]}
        result = executor_expectation_values(
            bool_array,
            basis_mapping,
            meas_basis_axis=0,
        ...
        self.assertEqual(len(result), 1)
        # All |0⟩ shots: projector |0><0| passes all → exp_val = 1
        np.testing.assert_allclose(result[0][0], 1.0, atol=1e-10)

    def test_projector_observable_mixed_shots_hits_else_branch(self):
        """Mixed shots against a projector observable should cause some shots been filtered.
        Half the shots pass the ``"0"`` projector → exp_val = 0.5.
        """
        # 2 shots |0⟩, 2 shots |1⟩ — half pass the "0" projector, half are filtered
        bool_array = np.array([[[False], [True], [False], [True]]], dtype=bool)
        proj_obs = SparseObservable.from_list([("0", 1.0)])
        basis_mapping = {Pauli("Z"): [proj_obs]}
        result = executor_expectation_values(
            bool_array,
        ...
        self.assertEqual(len(result), 1)
        # 2 out of 4 shots pass → exp_val = 2/4 = 0.5
        np.testing.assert_allclose(result[0][0], 0.5, atol=1e-10)

    def test_array_gamma_factor_non_with_no_pauli_signs(self):
        """``gamma_factor`` is not None while ``pauli_signs`` is None."""
        # Single basis, bool_array shape (1, 4, 2), meas_basis_axis=0.
        bool_array = np.zeros((1, 4, 2), dtype=bool)
        basis_mapping = {Pauli("ZZ"): [SparsePauliOp("ZZ")]}
...
class TestProjectToZ(unittest.TestCase):
    """Tests for :func:`_project_to_z`."""

    def test_z_maps_to_z(self):
        np.testing.assert_array_equal(_project_to_z("Z"), ["Z"])

    def test_x_maps_to_z(self):
        np.testing.assert_array_equal(_project_to_z("X"), ["Z"])

    def test_y_maps_to_z(self):
        np.testing.assert_array_equal(_project_to_z("Y"), ["Z"])

    def test_i_maps_to_i(self):
        np.testing.assert_array_equal(_project_to_z("I"), ["I"])

    def test_zero_maps_to_zero(self):
        np.testing.assert_array_equal(_project_to_z("0"), ["0"])

    def test_one_maps_to_one(self):
        np.testing.assert_array_equal(_project_to_z("1"), ["1"])

    def test_plus_maps_to_zero(self):
        np.testing.assert_array_equal(_project_to_z("+"), ["0"])

    def test_minus_maps_to_one(self):
        np.testing.assert_array_equal(_project_to_z("-"), ["1"])

    def test_r_maps_to_zero(self):
        np.testing.assert_array_equal(_project_to_z("r"), ["0"])

    def test_l_maps_to_one(self):
        np.testing.assert_array_equal(_project_to_z("l"), ["1"])

    def test_mixed_string(self):
        """ZXY → ZZZ; 0r1 → 001."""
        np.testing.assert_array_equal(_project_to_z("ZXY"), ["Z", "Z", "Z"])
        np.testing.assert_array_equal(_project_to_z("0r1"), ["0", "0", "1"])

    def test_all_identity_string(self):
        np.testing.assert_array_equal(_project_to_z("III"), ["I", "I", "I"])

    def test_returns_ndarray(self):
        result = _project_to_z("Z")
        self.assertIsInstance(result, np.ndarray)

    def test_char_to_z_chars_constants(self):
        """All entries declared in CHAR_TO_Z_CHARS must map correctly."""
        for ch, expected in CHAR_TO_Z_CHARS.items():
            with self.subTest(ch=ch):
                np.testing.assert_array_equal(_project_to_z(ch), [expected])


class TestComputeExpVal(unittest.TestCase):
    """Tests for :func:`compute_exp_val`."""

    # ------------------------------------------------------------------
    # Basic Z-type observables
    # ------------------------------------------------------------------

    def test_all_zeros_z_gives_plus_one(self):
        """All |0⟩ shots → ⟨Z⟩ = +1."""
        data = np.zeros((1, 4, 1), dtype=bool)
        ev, _, _ = _compute_single_term_exp_val_with_twirl_variance("Z", data)
        np.testing.assert_allclose(ev, 1.0, atol=1e-10)

    def test_all_ones_z_gives_minus_one(self):
        """All |1⟩ shots → ⟨Z⟩ = -1."""
        data = np.ones((1, 4, 1), dtype=bool)
        ev, _, _ = _compute_single_term_exp_val_with_twirl_variance("Z", data)
        np.testing.assert_allclose(ev, -1.0, atol=1e-10)

    def test_half_half_z_gives_zero(self):
        """Half |0⟩, half |1⟩ → ⟨Z⟩ = 0."""
        data = np.array([[[False], [False], [True], [True]]], dtype=bool)
        ev, _, _ = _compute_single_term_exp_val_with_twirl_variance("Z", data)
        np.testing.assert_allclose(ev, 0.0, atol=1e-10)

    def test_identity_gives_one(self):
        """⟨I⟩ must always be 1 regardless of data."""
        data = np.ones((2, 4, 2), dtype=bool)
        ev, _, _ = _compute_single_term_exp_val_with_twirl_variance("II", data)
        np.testing.assert_allclose(ev, 1.0, atol=1e-10)

    def test_zz_all_zeros(self):
        """ZZ on |00⟩: (+1)(+1) = +1."""
        data = np.zeros((1, 4, 2), dtype=bool)
        ev, _, _ = _compute_single_term_exp_val_with_twirl_variance("ZZ", data)
        np.testing.assert_allclose(ev, 1.0, atol=1e-10)

    def test_zz_all_ones(self):
        """ZZ on |11⟩: (-1)(-1) = +1."""
        data = np.ones((1, 4, 2), dtype=bool)
        ev, _, _ = _compute_single_term_exp_val_with_twirl_variance("ZZ", data)
        np.testing.assert_allclose(ev, 1.0, atol=1e-10)

    def test_zi_qubit1_all_ones(self):
        """ZI (Z on qubit 1, I on qubit 0): qubit-1 always |1⟩ → ⟨ZI⟩ = -1.

        Note: project_to_z reverses before applying masks ([::-1]), so qubit-0
        in the label string corresponds to index 1 in the array (big-endian label).
        ZI → qubit 1 in array notation = index 1.
        """
        data = np.zeros((1, 4, 2), dtype=bool)
        data[:, :, 1] = True  # qubit index 1 always |1⟩
        ev, _, _ = _compute_single_term_exp_val_with_twirl_variance("ZI", data)
        np.testing.assert_allclose(ev, -1.0, atol=1e-10)

    # ------------------------------------------------------------------
    # Projector ("0"/"1") observable types
    # ------------------------------------------------------------------

    def test_projector_zero_all_zeros_pass(self):
        """All |0⟩ shots all pass the "0" projector → exp_val = 1 (all kept, eigenvalue +1)."""
        data = np.zeros((1, 4, 1), dtype=bool)
        ev, _, _ = _compute_single_term_exp_val_with_twirl_variance("0", data)
        np.testing.assert_allclose(ev, 1.0, atol=1e-10)

    def test_projector_zero_all_ones_filtered(self):
        """All |1⟩ shots are filtered by the "0" projector → exp_val = 0."""
        data = np.ones((1, 4, 1), dtype=bool)
        ev, _, _ = _compute_single_term_exp_val_with_twirl_variance("0", data)
        np.testing.assert_allclose(ev, 0.0, atol=1e-10)

    def test_projector_one_all_ones_pass(self):
        """All |1⟩ shots all pass the "1" projector → exp_val = 1."""
        data = np.ones((1, 4, 1), dtype=bool)
        ev, _, _ = _compute_single_term_exp_val_with_twirl_variance("1", data)
        np.testing.assert_allclose(ev, 1.0, atol=1e-10)

    def test_projector_one_all_zeros_filtered(self):
        """All |0⟩ shots are filtered by the "1" projector → exp_val = 0."""
        data = np.zeros((1, 4, 1), dtype=bool)
        ev, _, _ = _compute_single_term_exp_val_with_twirl_variance("1", data)
        np.testing.assert_allclose(ev, 0.0, atol=1e-10)
    ...
    def test_returns_three_numpy_arrays(self):
        data = np.zeros((2, 4, 2), dtype=bool)
        result = _compute_single_term_exp_val_with_twirl_variance("ZZ", data)
        self.assertEqual(len(result), 3)
        for arr in result:
            self.assertIsInstance(arr, np.ndarray)

    def test_variances_are_nonnegative(self):
        data = np.random.default_rng(42).integers(0, 2, size=(4, 8, 3), dtype=bool)
        _, ens_var, twirl_var = _compute_single_term_exp_val_with_twirl_variance("ZZZ", data)
        self.assertGreaterEqual(float(ens_var), 0.0)
        self.assertGreaterEqual(float(twirl_var), 0.0)

    # ------------------------------------------------------------------
    # Single randomization: twirl_variance == ensemble_variance
    # ------------------------------------------------------------------

    def test_single_randomization_twirl_eq_ensemble_variance(self):
        data = np.array([[[False, False], [True, True], [False, True], [True, False]]], dtype=bool)
        _, ens_var, twirl_var = _compute_single_term_exp_val_with_twirl_variance("ZZ", data)
        np.testing.assert_allclose(twirl_var, ens_var, atol=1e-10)

    # ------------------------------------------------------------------
    # PEC signs
    # ------------------------------------------------------------------

    def test_signs_flip_exp_val(self):
        """All-ones signs (odd parity) on all-zeros data should flip +1 to -1."""
        data = np.zeros((2, 4, 1), dtype=bool)
        signs = np.ones((2, 1), dtype=bool)  # parity = 1 for each randomization → flip
        ev, _, _ = _compute_single_term_exp_val_with_twirl_variance("Z", data, signs=signs)
        np.testing.assert_allclose(ev, -1.0, atol=1e-10)

    def test_even_parity_signs_do_not_flip(self):
        """Even-parity signs (sum mod 2 == 0) leave the expectation value unchanged."""
        data = np.zeros((2, 4, 1), dtype=bool)
        signs = np.zeros((2, 2), dtype=bool)  # parity = 0 for each randomization → no flip
        ev, _, _ = _compute_single_term_exp_val_with_twirl_variance("Z", data, signs=signs)
        np.testing.assert_allclose(ev, 1.0, atol=1e-10)


class TestExecutorExpectationValuesRescaleFactors(unittest.TestCase):
    """Regression tests for the ``rescale_factors`` path of ``executor_expectation_values``.

    The rescale path previously crashed on any use (the observables in the internal basis
    mapping are ``SparseObservable``, which has no ``paulis`` attribute), crashed on ``None``
    zero-operator placeholders, and could not flatten ragged per-observable factor lists.
    """

    def _shot_data(self, num_bases: int = 1) -> np.ndarray:
        """Return deterministic two-qubit shot data of shape ``(num_bases, shots, 2)``."""
        rng = np.random.default_rng(1234)
        return rng.integers(0, 2, size=(num_bases, 100, 2)).astype(bool)

    def test_single_term_observable_scaled_exactly(self):
        """A single-term observable's expectation value must scale by exactly its factor."""
        bool_array = self._shot_data()
        basis_mapping = {Pauli("ZZ"): [SparsePauliOp("ZI")]}
        (base,) = executor_expectation_values(bool_array, basis_mapping, meas_basis_axis=0)
        (scaled,) = executor_expectation_values(
            bool_array, basis_mapping, meas_basis_axis=0, rescale_factors={"ZI": 2.0}
        )
        np.testing.assert_allclose(scaled[0], 2.0 * base[0])
        np.testing.assert_allclose(scaled[1], 4.0 * base[1])

    def test_factors_applied_per_term(self):
        """Each term of a multi-term observable must be scaled by its own factor."""
        bool_array = self._shot_data()
        (ev_zi,) = executor_expectation_values(
            bool_array, {Pauli("ZZ"): [SparsePauliOp("ZI")]}, meas_basis_axis=0
        )
        (ev_iz,) = executor_expectation_values(
            bool_array, {Pauli("ZZ"): [SparsePauliOp("IZ")]}, meas_basis_axis=0
        )
        basis_mapping = {Pauli("ZZ"): [SparsePauliOp(["ZI", "IZ"])]}
        (scaled,) = executor_expectation_values(
            bool_array, basis_mapping, meas_basis_axis=0, rescale_factors={"ZI": 2.0, "IZ": 3.0}
        )
        np.testing.assert_allclose(scaled[0], 2.0 * ev_zi[0] + 3.0 * ev_iz[0])

    def test_none_placeholder_observable(self):
        """``None`` zero-operator placeholders in the basis mapping must not crash rescaling."""
        bool_array = self._shot_data(num_bases=2)
        basis_mapping = {
            Pauli("ZZ"): [SparsePauliOp("ZI"), None],
            Pauli("XX"): [None, SparsePauliOp("XX")],
        }
        base = executor_expectation_values(bool_array, basis_mapping, meas_basis_axis=0)
        scaled = executor_expectation_values(
            bool_array,
            basis_mapping,
            meas_basis_axis=0,
            rescale_factors={"ZI": 2.0, "XX": 5.0},
        )
        np.testing.assert_allclose(scaled[0][0], 2.0 * base[0][0])
        np.testing.assert_allclose(scaled[1][0], 5.0 * base[1][0])

    def test_ragged_observable_term_counts(self):
        """Observables with different term counts in one basis must not break the flatten."""
        bool_array = self._shot_data()
        basis_mapping = {Pauli("ZZ"): [SparsePauliOp(["ZI", "IZ", "ZZ"]), SparsePauliOp("ZZ")]}
        base = executor_expectation_values(bool_array, basis_mapping, meas_basis_axis=0)
        scaled = executor_expectation_values(
            bool_array,
            basis_mapping,
            meas_basis_axis=0,
            rescale_factors={"ZI": 1.0, "IZ": 1.0, "ZZ": 7.0},
        )
        # The second observable is a bare ZZ term, so it scales by exactly 7.
        np.testing.assert_allclose(scaled[1][0], 7.0 * base[1][0])

    def test_empirical_gamma_matches_explicit_gamma(self):
        """Empirical gamma (``gamma_factor=None``) must behave like an explicit gamma.

        Previously the per-basis loop multiplied the whole empirical gamma array, which still
        carries the measurement-basis axis, into each basis' means (and appended an axis to it
        on every iteration), raising a broadcast ``ValueError`` whenever ``pauli_signs`` was
        given without an explicit gamma. The failure did not depend on ``avg_axis``, so this
        checks every averaging mode, with one and with two measurement bases.
        """
        rng = np.random.default_rng(7)
        # Net sign +1 for every randomization, so the empirical gamma is exactly 1 and the
        # result must match ``gamma_factor=1.0`` elementwise:
        cases = {
            "one basis": (
                rng.integers(0, 2, size=(20, 1, 16, 2)).astype(bool),
                np.zeros((20, 1, 3), dtype=bool),
                None,
                {Pauli("ZZ"): [SparsePauliOp("ZI"), SparsePauliOp("IZ")]},
                (None, (0,), (0, 1)),
            ),
            "two bases": (  # axis 1 is the measurement-basis axis, so it cannot be averaged
                rng.integers(0, 2, size=(20, 2, 16, 2)).astype(bool),
                np.zeros((20, 2, 3), dtype=bool),
                1,
                {
                    Pauli("ZZ"): [SparsePauliOp("ZI"), None],
                    Pauli("XX"): [None, SparsePauliOp("XI")],
                },
                (None, (0,)),
            ),
        }
        for name, (bool_array, signs, meas_basis_axis, basis_mapping, avg_axes) in cases.items():
            for avg_axis in avg_axes:
                with self.subTest(case=name, avg_axis=avg_axis):
                    empirical = executor_expectation_values(
                        bool_array,
                        basis_mapping,
                        meas_basis_axis,
                        avg_axis=avg_axis,
                    ...
                    explicit = executor_expectation_values(
                        bool_array,
                        basis_mapping,
                        meas_basis_axis,
                        avg_axis=avg_axis,
                    ...
                    for (m_emp, v_emp), (m_exp, v_exp) in zip(empirical, explicit, strict=True):
                        np.testing.assert_allclose(m_emp, m_exp)
                        np.testing.assert_allclose(v_emp, v_exp)

    def test_empirical_gamma_with_postselection_runs(self):
        """The full PEC-with-postselection combination (signs, no gamma, mask) must run."""
        rng = np.random.default_rng(8)
        bool_array = rng.integers(0, 2, size=(20, 1, 16, 2)).astype(bool)
        mask = rng.random((20, 1, 16)) < 0.5
        signs = rng.integers(0, 2, size=(20, 1, 3)).astype(bool)
        basis_mapping = {Pauli("ZZ"): [SparsePauliOp("ZI"), SparsePauliOp("IZ")]}
        masked = executor_expectation_values(
            bool_array,
            basis_mapping,
            None,
            avg_axis=(0, 1),
    ...
    def test_rescale_with_postselect_mask(self):
        """Factors stay keyed by the caller's labels when postselection appends its bit.

        Postselection internally appends a ``1`` projector bit to every observable; the
        rescale lookup must still use the original labels (previously ``KeyError: '1ZI'``).
        """
        bool_array = self._shot_data()
        basis_mapping = {Pauli("ZZ"): [SparsePauliOp("ZI")]}
        mask = np.ones(bool_array.shape[:-1], dtype=bool)
        mask[..., ::3] = False
        (base,) = executor_expectation_values(
            bool_array, basis_mapping, meas_basis_axis=0, postselect_mask=mask
        )
        (scaled,) = executor_expectation_values(
            bool_array,
            basis_mapping,
            meas_basis_axis=0,
            postselect_mask=mask,
            rescale_factors={"ZI": 2.0},
        )
        np.testing.assert_allclose(scaled[0], 2.0 * base[0])

    def test_rescale_with_pauli_signs(self):
        """Factors stay keyed by the caller's labels when PEC signs append their bit."""
        bool_array = self._shot_data()
        basis_mapping = {Pauli("ZZ"): [SparsePauliOp("ZI")]}
        signs = np.zeros((1, 3), dtype=bool)
        (base,) = executor_expectation_values(
            bool_array, basis_mapping, meas_basis_axis=0, pauli_signs=signs, gamma_factor=1.0
        )
        (scaled,) = executor_expectation_values(
            bool_array,
            basis_mapping,
